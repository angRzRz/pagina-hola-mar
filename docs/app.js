...
const messages = {
  es: {
    ...
    askBudget: "¿Cuál es tu presupuesto máximo en euros?",
    ...
    finishBuyer:
      "Si te interesa alguna, dime el código y un agente te contactará.",
    finishSeller:
  ...
  en: {
    ...
    askBudget: "What's your maximum budget in euros?",
    ...
    finishBuyer:
      "If any interest you, share the code and an agent will contact you.",
...
function listMatches() {
  ...
  if (!matches.length) {
    addMessage("bot", getText("noMatches"));
    addMessage("bot", getText("finishBuyer"));
    return;
  }
  addMessage("bot", getText("matchesIntro"));
  matches.slice(0, 5).forEach((property) => {
    addMessage("bot", formatProperty(property));
  });
  addMessage("bot", getText("finishBuyer"));
}
